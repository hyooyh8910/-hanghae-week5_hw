...
function Write() {
  ...
  //   const uploaded_file = await uploadBytes(ref(storage, 
  ...
  const uploadFB = async (e) => { 
    ...
    uploadBytes(storageRef, fileInput).then((snapshot) => { console.log(file); }); };
  ...
  return (
    <WriteDivBox style={{
      ...
      width: '155vh', height: '130vh', flexDirection: 'column'
    }}>
      <h1 style={{ marginTop: '100px' }}><b>게시글 작성</b></h1>
      <WriteDiv style={{
        display: 'flex', justifyContent: 'center', alignItems: 'center',
        width: '600px', height: '5px', marginLeft: '180px',
      }}>


        이미지: <input ref={fileInput} type="file" onChange={(e) => {
      ...
      <div className="img_box">
        <div className="preview">
          {imageSrc && <img src={imageSrc} alt="preview-img" style={{width: 150}} />}
        </div>
      </div>
